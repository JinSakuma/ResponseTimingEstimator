...
from src.models.encoder.timing_encoder7 import TimingEncoder
...
class TimingEstimator(nn.Module):
    ...
    def create_models(self):
        ...
        encoding_dim = self.config.model_params.acoustic_encoding_dim \
                        + self.config.model_params.semantic_encoding_dim*2 \
    ...
    def forward(self, batch, split='train'):
        ...
        r_t, texts2 = self.timing_encoder(feats, idxs, input_lengths, indices, split)        
        r_s = self.semantic_encoder(texts)
        r_s2 = self.semantic_encoder(texts2)
        
        embs = torch.cat([r_s, r_a, r_t, r_s2], dim=-1)              
