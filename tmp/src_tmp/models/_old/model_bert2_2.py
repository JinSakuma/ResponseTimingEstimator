...
#             loss = loss+self.timing_model.get_loss(output[:input_lengths[i]], targets[i][:input_lengths[i]])
        ...
        outputs = self.timing_model(embs, vad_preds, input_lengths, indices, split)
        for i in range(batch_size):
            uu = vad[i][:input_lengths[i]].detach().cpu().numpy()
            timing = np.where(targets[i].detach().cpu().numpy()==1)[0][0]
            t_ups = np.where(uu[1:]-uu[:-1]==1)[0]+1
            
            ipu_start_lists = t_ups[t_ups<timing]#[-1]
            if len(ipu_start_lists)>1:
                last_ipu_start = ipu_start_lists[-2]
            else:
                last_ipu_start = ipu_start_lists[-1]
            
            loss = loss+self.timing_model.get_loss(outputs[i][last_ipu_start:input_lengths[i]], targets[i][last_ipu_start:input_lengths[i]])
