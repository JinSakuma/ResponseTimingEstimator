...
#             last_ipu_agent = self.get_last_ipu(vad_agent)
    ...
    #             turn_agent = np.ones(len(turn_agent)) - turn_agent
            ...
            # text
            text_path = '_'.join(wav_path.replace('wav', 'phoneme').split('_')[:-1])+'.json'
            with open(text_path, 'r') as jf:
                text_dict = json.load(jf)
            
            text = text_dict['text']
            phon = text_dict['phoneme']
            idx = text_dict['idx']
...
def collate_fn(batch, pad_idx=46):
    #chs, offsets, is_barge_in, texts, feat_paths, wav_paths, vad, turn, last_ipu, targets, feats, indices = zip(*batch)
    #chs, texts, feat_paths, wav_paths, vad, turn, last_ipu, targets, feats, wavs = zip(*batch)
    paths, texts, phonemes, idxs, indices = zip(*batch)
    ...
    for i in range(batch_size):
        #l1 = len(feats[i])
        l1 = len(idxs[i])
        input_lengths.append(l1-1)
        
        # l2 = len(wavs[i])
        #wav_lengths.append(l2)
        
        idxs_.append(idxs[i]+[pad_idx]*(max_len-l1))
    ...
    return paths, texts, phonemes, idxs_, input_lengths, indices 
    # return chs, texts, vad_, turn_, last_ipu_, target_, feat_, input_lengths, offsets, indices, is_barge_in, #wav_, wav_lengths, wav_paths
