...
class ATRDataset(Dataset):
    def __init__(self, config, split='train'):
        ...
        self.asr_delay1 = 640 # 実際のASRの遅延 [ms]
        self.asr_delay2 = 100 # 実験用の仮のASRの遅延 [ms]
    ...
    def get_turn_info(self, file_name):
        # 各種ファイルの読み込み
        df_turns_path = os.path.join(DATAROOT, 'dataframe2/{}.csv'.format(file_name))
...
def collate_fn(batch):
    #chs, offsets, texts, feat_paths, wav_paths, vad, turn, last_ipu, targets, feats = zip(*batch)
    chs, offsets, texts, feat_paths, wav_paths, vad, turn, last_ipu, targets, feats, wavs = zip(*batch)
    ...
    return chs, text_, vad_, turn_, last_ipu_, target_, feat_, input_lengths, offsets, wav_, wav_lengths, wav_paths
