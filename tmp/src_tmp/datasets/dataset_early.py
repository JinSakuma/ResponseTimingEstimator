...
class ATRDataset(Dataset):
    ...
    #             turn_agent = np.ones(len(turn_agent)) - turn_agent
            ...
            # EoU先読みのためのターンラベルの調節
            turn_label = np.ones(len(turn_timing_target))
...
#             #turn[-pad:] = 0
            ...
            length = len(turn_label)
            target = np.zeros([length, self.N])

            for i in range(length):
                n = len(turn_label[i:i+self.N])
                tmp = np.zeros(self.N)
                tmp[:n] = turn_label[i:i+self.N]
                target[i] = tmp
...
def collate_fn(batch):
    chs, offsets, starts, ends, texts, feat_paths, wav_paths, vad, turn, last_ipu, targets, feats, indices = zip(*batch)
    ...
    target_ = torch.ones(batch_size, max_len, target_dim).long()*(-100)
    ...
    return chs, text_, vad_, turn_, last_ipu_, target_, feat_, input_lengths, offsets, indices, starts, ends, #wav_, wav_lengths, wav_paths
