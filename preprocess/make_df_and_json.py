...
columns = ["start", "end", "spk", "next_spk", "next_sys_uttr_start", "next_sys_uttr_end", "next_usr_uttr_start", "offset", "length"]
df_list = []
name_list = []
for j, file_path in enumerate(tqdm(file_list)): 
    ...
    #     offset = start-eou
        ...
        if spk==0:
            df_vad_spk = df_vad[df_vad['spk']==1]
        else:
            df_vad_spk = df_vad[df_vad['spk']==0]
        
        start = df['start'].iloc[i]
        end = df['end'].iloc[i]
        df_tmp = df_vad_spk[(df_vad_spk['end']>start) & (df_vad_spk['start']<start)]
        ...
        if len(df_tmp)>0:
            ...
            offset=start-df_vad_spk[df_vad_spk['end']<=start].iloc[-1]['end']
            pre_spk = df_vad_spk[df_vad_spk['end']<=start].iloc[-1]['spk']
            pre_end = df_vad_spk[df_vad_spk['end']<=start].iloc[-1]['end']
        ...
        next_sys_uttr_end.append(end)
        next_usr_uttr.append(next_usr)
        offsets.append(offset)

    df['offset'] = offsets+[100000]
    df['next_spk'] = next_spk+[-1]
    df['next_sys_uttr_start'] = next_sys_uttr+[100000]
    df['next_sys_uttr_end'] = next_sys_uttr_end+[100000]
    df['next_usr_uttr_start'] = next_usr_uttr+[100000]
    df = df[columns]
#     df_list.append(df)
#     name_list.append(name)    
    
    df_path = '/mnt/aoni04/jsakuma/data/ATR-Fujie/dataframe3/{}.csv'.format(name)
    df_vad_path = '/mnt/aoni04/jsakuma/data/ATR-Fujie/vad/{}.csv'.format(name)
    df.to_csv(df_path, encoding='utf-8', index=False)
    #df_vad.to_csv(df_vad_path, encoding='utf-8', index=False)