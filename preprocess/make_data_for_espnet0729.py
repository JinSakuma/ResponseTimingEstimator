...
def get_text_file(name):
    ...
    columns = ['start1', 'start2', 'end1', 'end2', 'text']
...
if __name__ == "__main__":
    ...
    for split in ['train']:
        ...
        path_text = os.path.join(root, 'data_offset100/{}/text'.format(split))
        path_wavscp = os.path.join(root, 'data_offset100/{}/wav.scp'.format(split))
        path_utt2spk = os.path.join(root, 'data_offset100/{}/utt2spk'.format(split))
        
        os.makedirs(path_text.replace('text', ''), exist_ok=True)
        #print(path_text)
        
        file_names_list = file_dict[split]
        for idx, file_name in enumerate(tqdm(file_names_list)):
            df_text_user, _ = get_text_file(file_name) 
            wavpath = os.path.join(root.replace('/asr', ''), 'wav_mono/{}_user.wav'.format(file_name))
            cnt = 0
            for i in range(len(df_text_user)):
                ...
                wav_out_dir = os.path.join(root, 'wav_offset100', file_name)
