...
from src.models.asr.e2e_asr_common import get_vgg2l_odim
from src.models.asr.nets_utils import make_pad_mask, to_device
...
class RNNP(torch.nn.Module):
    ...
    def forward(
        ...
        for layer in range(self.elayers):
            ...
            tlen = rnn_input.size(1)
            pack_rnn_input = pack_padded_sequence(
                rnn_input, rnn_len.cpu(), batch_first=True, enforce_sorted=False
            )
            ...
            pad_rnn_output, rnn_len = pad_packed_sequence(
                pack_rnn_output, batch_first=True, total_length=tlen
            )
