...
class TimingEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        if is_use_n_word:
            ...
            self.lm = TransformerLM(lm_config, device)
    ...
    def lm_generate(self, inp):
        ...
        with torch.no_grad():            
            for i in range(self.max_n_word):
                out = self.lm.lm(prev.unsqueeze(0))
                ...
                prev = torch.cat([prev, pred[-1].unsqueeze(0)])
    ...
    def get_n_best_length(self, inp):        
        endnodes = []
        with torch.no_grad():        
            decoder_output = self.lm.lm(inp.unsqueeze(0))
            ...
            for i in range(len(indexes)):        
                n = BeamSearchNode(inp, None, indexes[i], prob_[i], 1)
                cur_nodes.put((-prob_[i], i+100*(0+1), n))

            cnt = 1
            while True:
                if cnt >= self.beam_width:
                    for bw in range(self.beam_width-n_end):
                ...
                for bw in range(self.beam_width-n_end):
                    ...
                    prev_inp = cur_node.prevInp
                    new_inp = torch.cat([prev_inp, idx.unsqueeze(0)])
                    decoder_output = self.lm.lm(new_inp.unsqueeze(0))
                    prob = torch.softmax(decoder_output, dim=-1)
                    ...
                    for i in range(len(indexes)):                                    
                        ...
                        n = BeamSearchNode(new_inp, cur_node, indexes[i], cur_node.prob+prob_[i]/float(cnt+1), cnt+1)
                        try:
...
class BeamSearchNode(object):
    def __init__(self, prevInp, previousNode, wordId, prob, length):
        
        self.prevInp = prevInp
