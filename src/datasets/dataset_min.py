...
class ATRDataset(Dataset):
    def __init__(self, config, split='train', speaker_list=None, scale=0.1):
        self.config = config
        ...
        data_num = int(len(self.file_names)*scale)
        self.file_names = self.file_names[:data_num]
...
def get_dataset(config, split="train", speaker_list=None, scale=0.1):
    dataset = ATRDataset(config, split, speaker_list, scale)
    return dataset
