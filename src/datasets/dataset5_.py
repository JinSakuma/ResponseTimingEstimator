...
class ATRDataset(Dataset):
    def __init__(self, config, split='train', speaker_list=None, task='timing'):
        self.config = config
    ...
    def get_turn_info(self, file_name, task):
        # 各種ファイルの読み込み
...
#                 target[i] = tmp
    ...
    def get_data(self, task):
        ...
        for file_name in tqdm(self.file_names):
            data += self.get_turn_info(file_name, task)
...
def collate_fn(batch):
    ...
    for i in range(batch_size):
        ...
        if kanas[0] is not None:
            kana_.append(kanas[i]+['[PAD]']*(max_len-l1))
    ...
    return chs, text_, kana_, idxs, vad_, turn_, last_ipu_, target_, feat_, input_lengths, offsets, indices, is_barge_in, #wav_, wav_lengths, wav_paths
...
def get_dataset(config, split="train", speaker_list=None, task='timing'):
    dataset = ATRDataset(config, split, speaker_list, task)
    return dataset
